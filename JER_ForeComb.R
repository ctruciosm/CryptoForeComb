...
cryptocurrency = "./BTC"  # Options are "./BTC", "./ETH" and "./LTC"
end_date = "2021-08-14"
...
# inES1_CS = read.csv("inES1_CS.csv")[,-1] + matrix(rep(mu,length(ret)), ncol = length(ret), byrow = TRUE)
...
## Formulas
formula2 =  Formula::as.Formula('r ~ VaRGAS2 + VaRMSGARCH2 + VaRBoot2 | ESGAS2 + ESMSGARCH2 + ESBoot2')
formula5 =  Formula::as.Formula('r ~ VaRGAS5 + VaRMSGARCH5 + VaRBoot5 | ESGAS5 + ESMSGARCH5 + ESBoot5')
formula10 = Formula::as.Formula('r ~ VaRGAS10 + VaRMSGARCH10 + VaRBoot10 | ESGAS10 + ESMSGARCH10 + ESBoot10')

## OoS VaR and ES
### We wrotw 0, but can put any value, this value is not used in the predict function
df_OoS_02 = data.frame(0,VaR2, ES2)
colnames(df_OoS_02) = c("r","VaRGAS2", "VaRMSGARCH2", "VaRBoot2","ESGAS2", "ESMSGARCH2", "ESBoot2")
df_OoS_05 = data.frame(0,VaR5, ES5)
colnames(df_OoS_05) = c("r","VaRGAS5", "VaRMSGARCH5", "VaRBoot5","ESGAS5", "ESMSGARCH5", "ESBoot5")
df_OoS_10 = data.frame(0,VaR10, ES10)
colnames(df_OoS_10) = c("r","VaRGAS10", "VaRMSGARCH10", "VaRBoot10","ESGAS10", "ESMSGARCH10", "ESBoot10")

VaR_JER= matrix(0, ncol = 3, nrow = OoS)
ES_JER = matrix(0, ncol = 3, nrow = OoS)
for (i in 1:OoS){
  print(i)
  df2 = data.frame(r =  crypto$ret[i:(InS+i-1)], VaRGAS2 = inVaR2_GAS[,i], VaRMSGARCH2 = inVaR2_MS[,i], VaRBoot2 = inVaR2_Boot[,i],
                   ESGAS2 = inES2_GAS[,i], ESMSGARCH2 = inES2_MS[,i], ESBoot2 = inES2_Boot[,i])
  df5 = data.frame(r =  crypto$ret[i:(InS+i-1)], VaRGAS5 = inVaR5_GAS[,i], VaRMSGARCH5 = inVaR5_MS[,i], VaRBoot5 = inVaR5_Boot[,i],
                   ESGAS5 = inES5_GAS[,i], ESMSGARCH5 = inES5_MS[,i], ESBoot5 = inES5_Boot[,i])
  df10= data.frame(r =  crypto$ret[i:(InS+i-1)], VaRGAS10 = inVaR10_GAS[,i], VaRMSGARCH10 = inVaR10_MS[,i], VaRBoot10 = inVaR10_Boot[,i],
                   ESGAS10 = inES10_GAS[,i], ESMSGARCH10 = inES10_MS[,i], ESBoot10 = inES10_Boot[,i]) 
  ...
  VaR_JER[i,] = c(pred_comb_02[1], pred_comb_05[1], pred_comb_10[1])
  ES_JER[i,] = c(pred_comb_02[2], pred_comb_05[2], pred_comb_10[2])   
}

colnames(VaR_JER) = c("VaR_JER2", "VaR_JER5", "VaR_JER10")
colnames(ES_JER) = c("ES_JER2", "ES_JER5", "ES_JER10")
