...
for (j in 1:3){
  ...
  omegaini5 = rep(rep(1/M,M),2)  
    ...
    for (i in 1:OoS){
      #### RSC  
      lambda1 = RSC_opt(parini = lini1, VaR = cbind(inVaR1_Boot[,i],inVaR1_GAS[,i], inVaR1_MS[,i]), ES = cbind(inES1_Boot[,i],inES1_GAS[,i], inES1_MS[,i]), r =  crypto$ret[i:(InS+i-1)], alpha = a1, S = S_)
      lini1 = lambda1
      lambda2 = RSC_opt(parini = lini2, VaR = cbind(inVaR2_Boot[,i],inVaR2_GAS[,i], inVaR2_MS[,i]), ES = cbind(inES2_Boot[,i],inES2_GAS[,i], inES2_MS[,i]), r =  crypto$ret[i:(InS+i-1)], alpha = a2, S = S_)
      lini2 = lambda2
      lambda5 = RSC_opt(parini = lini5, VaR = cbind(inVaR5_Boot[,i],inVaR5_GAS[,i], inVaR5_MS[,i]), ES = cbind(inES5_Boot[,i],inES5_GAS[,i], inES5_MS[,i]), r =  crypto$ret[i:(InS+i-1)], alpha = a5, S = S_)
      lini5 = lambda5
      ...
      #### MSC
      omega1MSC[i,] = MSC_opt(parini = omegaini1, VaR = cbind(inVaR1_Boot[,i],inVaR1_GAS[,i], inVaR1_MS[,i]), ES = cbind(inES1_Boot[,i],inES1_GAS[,i], inES1_MS[,i]), r = crypto$ret[i:(InS+i-1)], alpha = a1, S = S_)
      omega2MSC[i,] = MSC_opt(parini = omegaini2, VaR = cbind(inVaR2_Boot[,i],inVaR2_GAS[,i], inVaR2_MS[,i]), ES = cbind(inES2_Boot[,i],inES2_GAS[,i], inES2_MS[,i]), r = crypto$ret[i:(InS+i-1)], alpha = a2, S = S_)
      omega5MSC[i,] = MSC_opt(parini = omegaini5, VaR = cbind(inVaR5_Boot[,i],inVaR5_GAS[,i], inVaR5_MS[,i]), ES = cbind(inES5_Boot[,i],inES5_GAS[,i], inES5_MS[,i]), r = crypto$ret[i:(InS+i-1)], alpha = a5, S = S_)
